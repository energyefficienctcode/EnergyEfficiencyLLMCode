...
class Solution {
    public int medianOfUniquenessArray(int[] nums) {
        int n = nums.length;
        Set<Integer> uniquenessSet = new HashSet<>();
        List<Integer> uniquenessArray = new ArrayList<>();

        for (int i = 0; i < n; i++) {
            uniquenessSet.clear();
            for (int j = i; j < n; j++) {
                uniquenessSet.add(nums[j]);
                uniquenessArray.add(uniquenessSet.size());
            }
        }

        Collections.sort(uniquenessArray);
        int size = uniquenessArray.size();
        return uniquenessArray.get((size - 1) / 2);
    }
}

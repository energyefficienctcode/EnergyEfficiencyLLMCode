class Solution(object):
    def minimumValueSum(self, nums, andValues):
        ...
        # Helper function to check if a subarray can match the given andValue
        def can_form_subarray(start, end, andValue):
            result = nums[start]
            for i in range(start + 1, end + 1):
                result &= nums[i]
            return result == andValue
        ...
        for i in range(1, n + 1):
            for j in range(1, m + 1):
                for k in range(i):
                    if can_form_subarray(k, i - 1, andValues[j - 1]):

...
class Solution {
    public long maximumValueSum(int[] nums, int k, int[][] edges) {
        ...
        // Identify beneficial nodes where flipping increases the value
        boolean[] beneficial = new boolean[n];
        long baseSum = 0;
        for(int i = 0; i < n; i++){
            baseSum += nums[i];
            long flipped = (long)(nums[i] ^ k);
            if(flipped > nums[i]){
                beneficial[i] = true;
            }
        }
        ...
        // Perform DFS to find maximum matching among beneficial nodes
        for(int i = 0; i < n; i++){
            if(!visited[i]){
                dfs(i, adj, beneficial, matched, nums, k, visited, totalBenefitWrapper -> {
                    totalBenefit += totalBenefitWrapper[0];
                });
            }
        }
        
        return baseSum + totalBenefit;
    }
    
    private void dfs(int node, List<List<Integer>> adj, boolean[] beneficial, boolean[] matched, int[] nums, int k, boolean[] visited, Consumer<long[]> callback){
        visited[node] = true;
        for(int neighbor : adj.get(node)){
            if(!visited[neighbor]){
                dfs(neighbor, adj, beneficial, matched, nums, k, visited, callback);
                if(beneficial[node] && beneficial[neighbor] && !matched[node] && !matched[neighbor]){
                    matched[node] = true;
                    matched[neighbor] = true;
                    long benefit = ((long)(nums[node] ^ k) - nums[node]) + ((long)(nums[neighbor] ^ k) - nums[neighbor]);
                    callback.accept(new long[]{benefit});
                }
            }
        }
    }

...
class Solution {
    public long countSubarrays(int[] nums, int k) {
        ...
        for(int num : nums){
            ...
            current.put(num, current.getOrDefault(num, 0L) + 1);
            
            // Extend existing subarrays by including the current number
            for(Map.Entry<Integer, Long> entry : prev.entrySet()){
                int andValue = entry.getKey() & num;
                current.put(andValue, current.getOrDefault(andValue, 0L) + entry.getValue());
            }
            
            // If any subarray has an AND equal to k, add its count to the result
            if(current.containsKey(k)){
                result += current.get(k);
            }
            
            // Update the previous map for the next iteration
            prev = current;
        }

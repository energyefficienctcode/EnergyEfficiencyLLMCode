...
public:
    int maxScore(vector<vector<int>>& grid) {
        ...
        // Use a set to ensure the values are unique across selected cells
        ...
        // Greedily select the maximum possible unique values from each row
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                if (selectedValues.find(grid[i][j]) == selectedValues.end()) {
                    selectedValues.insert(grid[i][j]);
                    maxScore += grid[i][j];
                    break;
                }
            }

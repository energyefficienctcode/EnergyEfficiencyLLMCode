...
class Solution {
    public int maxPathLength(int[][] coordinates, int k) {
        ...
        int[] dp = new int[n]; // dp[i] stores the longest path ending at coordinates[i]
        Arrays.fill(dp, 1); // Each point is a path of at least length 1
        int maxLength = 1;

        // Sort the coordinates based on both x and y
        Arrays.sort(coordinates, (a, b) -> {
            if (a[0] == b[0]) return a[1] - b[1];
            return a[0] - b[0];
        });

        // Dynamic programming to calculate the longest increasing path
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < i; j++) {
        ...
        return dp[k]; // Return the longest path length that contains the coordinates[k]
    }
}

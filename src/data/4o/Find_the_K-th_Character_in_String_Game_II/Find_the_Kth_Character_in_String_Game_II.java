class Solution {
    public char kthCharacter(long k, int[] operations) {
        ...
        for (int operation : operations) {
            ...
            if (operation == 0) {
                // Append a copy of the word to itself
                word.append(word);
            } else if (operation == 1) {
                // Generate new string by incrementing each character, then append it to the word
            ...
            // If word length exceeds k, we can stop early as we only need the k-th character
            if (word.length() >= k) {
                break;
            }
        }

        return word.charAt((int) (k - 1)); // k is 1-based, convert it to 0-based index
    }
}

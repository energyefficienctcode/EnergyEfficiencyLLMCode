...
class Solution {
    public long minDamage(int power, int[] damage, int[] health) {
        ...
        // Max heap to prioritize enemies based on their damage output
        PriorityQueue<int[]> maxHeap = new PriorityQueue<>((a, b) -> b[0] - a[0]);

        // Add all enemies to the heap with their corresponding damage and health
        for (int i = 0; i < n; i++) {
            maxHeap.add(new int[]{damage[i], health[i]});
        }

        // While there are enemies alive
        while (!maxHeap.isEmpty()) {
            int[] enemy = maxHeap.poll();
            int enemyDamage = enemy[0];
            int enemyHealth = enemy[1];

            // Calculate how many seconds this enemy will stay alive
            int rounds = (enemyHealth + power - 1) / power; // ceil(health / power)

            // Bob will take damage for these rounds
            totalDamage += (long) enemyDamage * rounds;
        }

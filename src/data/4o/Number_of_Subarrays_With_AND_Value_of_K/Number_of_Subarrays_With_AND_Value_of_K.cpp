...
public:
    long long countSubarrays(vector<int>& nums, int k) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = i; j < n; ++j) {
                // Compute the AND for the current subarray
                andResult &= nums[j];
                
                // If the AND result equals k, increment the count
                if (andResult == k) {
                    count++;
                }
                
                // If the result is already less than k, no need to continue this subarray
                if (andResult < k) break;
            }
        }

class Solution(object):
    def stringIndices(self, wordsContainer, wordsQuery):
        ...
        :rtype: List[int]
        """
        def longest_common_suffix_len(s1, s2):
        ...
        for query in wordsQuery:
            ...
            for i, container_word in enumerate(wordsContainer):
                suffix_len = longest_common_suffix_len(container_word, query)
                
                if suffix_len > max_suffix_len or (suffix_len == max_suffix_len and len(container_word) < len(wordsContainer[best_index])):

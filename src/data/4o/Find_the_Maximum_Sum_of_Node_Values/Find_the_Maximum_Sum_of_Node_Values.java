...
class Solution {
    ...
    public long maximumValueSum(int[] nums, int k, int[][] edges) {
        ...
        // Get the sum without any operations
        long originalSum = 0;
        for (int num : nums) {
            originalSum += num;
        }

        // Calculate the sum with XOR operation
        long xorSum = dfs(0, nums, k);

        // Return the maximum of the original sum and the sum after XOR operations
        return Math.max(originalSum, xorSum);
    }

    private long dfs(int node, int[] nums, int k) {
        visited[node] = true;
        long xorSum = nums[node] ^ k;
        
        for (int neighbor : adjList.get(node)) {
            if (!visited[neighbor]) {
                xorSum += dfs(neighbor, nums, k);
            }
        }

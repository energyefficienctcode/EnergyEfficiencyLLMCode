...
public:
    int numDistinct(string s, string t) {
        ...
        // dp[i][j] will store the number of distinct subsequences of s[0..i-1] that match t[0..j-1]
        vector<vector<long>> dp(m + 1, vector<long>(n + 1, 0));

        // Every string has exactly one subsequence that matches an empty string
        for (int i = 0; i <= m; i++) {
            dp[i][0] = 1;
        }

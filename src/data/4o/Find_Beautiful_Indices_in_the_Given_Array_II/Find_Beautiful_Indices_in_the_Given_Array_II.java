...
class Solution {
    public List<Integer> beautifulIndices(String s, String a, String b, int k) {
        ...
        // Find all starting indices where the substring is equal to 'a'
        ...
        // Find all starting indices where the substring is equal to 'b'
        ...
        // Check each index in aIndices and find a corresponding index in bIndices such that |j - i| <= k
        for (int i : aIndices) {
            for (int j : bIndices) {
                if (Math.abs(j - i) <= k) {
                    beautifulIndices.add(i);
                    break; // We only need one valid j for each i
                }
            }
        }

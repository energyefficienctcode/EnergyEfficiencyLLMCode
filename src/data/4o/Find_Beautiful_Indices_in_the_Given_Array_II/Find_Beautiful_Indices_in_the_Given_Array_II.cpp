...
public:
    vector<int> beautifulIndices(string s, string a, string b, int k) {
        ...
        // Store the indices where string "b" matches
        vector<int> bIndices;
        ...
        // Find beautiful indices for string "a"
        for (int i = 0; i <= n - aLen; i++) {
            if (s.substr(i, aLen) == a) {
                // Check if there is a valid "j" such that |j - i| <= k
                for (int j : bIndices) {
                    if (abs(j - i) <= k) {
                        result.push_back(i);
                        break; // No need to check further once a valid j is found
                    }
                }
            }
        }
